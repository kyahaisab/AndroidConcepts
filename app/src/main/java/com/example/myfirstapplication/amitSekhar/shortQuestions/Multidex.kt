...
you analyze the APK you will see that there are index files and
it is named as classes.dex so you can check classes.dex classes2.dex something like that. So in our APK we can haven n
number of dex files one two three four five and so on that number depends on the number of methods present in our code base
or the libraries or the framework that you have used to build the project so that will depend on the number of methods
that you you have used to build the project
...
so so basically we can analyze the APK and check how many Dex files are present in our APK so now you know how to check the number of Dex file in an APK

Always analyze apk where you have applied progaurd or R8 rules. if your app has more than one desk file it means that we call it as multi decks
